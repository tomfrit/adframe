...
use de::adframe::core::XmlSnippetGenerator;
use de::adframe::core::XmlHtmlGenerator;
$VERSION=20061123;
1;
sub new
{
...
sub generate
{
	...
	if (-e "$ENV{ADFRAME_DOC_PATH}/$self->{doc}\.xhtml")
	{
		my $xmlHtml = new XmlHtmlGenerator($ENV{ADFRAME_DOC_PATH},$self->{doc},$self->{filedir});
		$xmlHtml->parse();
		$xmlHtml->store();
	}
...
sub _parseDynamicSnippets
{
	...
	while ($go)
	{
		...
		{
			$thisname       = $1;
			$self->{dict}->{$thisname}->{htmltype} = 'dynamicsnippet' if (($thisname ne 'id')&& ($thisname ne 'doc') && ($thisname ne ''));
			
			$self->{docline} =~ /<dynamicsnippet\s+name=\s*\"$thisname\".*?src=\"(.*?)\".*?>/;
			my $snippetScr = '';

			if ($self->{docline} =~ /<dynamicsnippet\s+name=\s*\"$thisname\".*?src=\"(.*?)\".*?>/)
			{
				$snippetScr = $1;
				$snippetScr	=~ s/^\///;	
			}		
			
			$self->{dict}->{$thisname}->{replace} = $snippetScr.'/'.$main::in{$thisname};
			$self->{dynamicsnippet}->{"$thisname"}= $self->{dict}->{$thisname}->{replace};
			$self->_getDynamicSnippet($thisname,$self->{dict}->{$thisname}->{replace});
			$dummy_line=~ s/<dynamicsnippet\s+name=\s*\"(.*?)\".*?>//gi;	
			if ($self->{dynamicsnippet}->{"$thisname"} ne '')
			{
				$self->{docline} =~ s/(<dynamicsnippet\s+name=\s*\"$thisname\").*?src=\".*?\"(.*?)>/$1$2>/ig;
				$self->{docline} =~ s/<dynamicsnippet\s+name=\s*\"$thisname\".*?>/$self->{dynamicsnippet}->{"$thisname"}/ig;				
			}
		}
		else
		{
...
sub _getDynamicSnippet
{
	...
	if (-e "$ENV{ADFRAME_DOC_PATH}/$src$filename\.snp")
	{
		if (open(DSNP, "$ENV{ADFRAME_DOC_PATH}/$filename\.snp"))
		{
			while (<DSNP>)
			{
				$self->{dynamicsnippet}->{"$filename"}.=$_;
			}
			close(FILE);
			$ok = 1;
		}
...
sub _modifyDoc
{
	...
	# neu:	
	$self->{docline} =~ s/(\<\s*form.*?[^-]\>)/$1\n<input type=\"hidden\" name=\"id\" value=\"<!-- #id# -->\"\>/ig;
