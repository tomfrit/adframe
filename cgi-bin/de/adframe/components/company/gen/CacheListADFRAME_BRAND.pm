package CacheListADFRAME_BRAND;
...
sub getADFRAME_BRAND
{
	...
	{
		$this->dbGetADFRAME_BRAND($condition,$orderBy,$start,$limit);
...
sub getCountADFRAME_BRAND
{
	...
	{
		$this->dbGetCountADFRAME_BRAND($condition);
...
sub dbGetADFRAME_BRAND
{
	...
	my $start			= shift;
	my $limit			= shift;
	require de::adframe::components::company::gen::ListADFRAME_BRAND;
	my $dblist = ListADFRAME_BRAND->new();		
	$this->{lines} = $dblist->getADFRAME_BRAND($condition,$orderBy,$start,$limit); 
}

sub dbGetCountADFRAME_BRAND
{
	my $this			= shift;
	my $condition		= shift;
	require de::adframe::components::company::gen::ListADFRAME_BRAND;
	my $dblist = ListADFRAME_BRAND->new();	
	$this->{count} = $dblist->getCountADFRAME_BRAND($condition);
}
sub readFromFile
{
	my $this	= shift;
...
sub addMessage
{
	my $this	= shift;
	my $mess	= shift;
	$this->{message}.='ClassADFRAME_BRAND: '.$mess."\n";   
}
1;