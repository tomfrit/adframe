package CacheListADFRAME_USERCOMPANYBRAND;
...
sub getADFRAME_USERCOMPANYBRAND
{
	...
	{
		$this->dbGetADFRAME_USERCOMPANYBRAND($condition,$orderBy,$start,$limit);
...
sub getCountADFRAME_USERCOMPANYBRAND
{
	...
	{
		$this->dbGetCountADFRAME_USERCOMPANYBRAND($condition);
...
sub dbGetADFRAME_USERCOMPANYBRAND
{
	my $this			= shift;
	my $condition		= shift;
	my $orderBy			= shift;
	my $start			= shift;
	my $limit			= shift;
	require de::adframe::components::company::gen::ListADFRAME_USERCOMPANYBRAND;
	my $dblist = ListADFRAME_USERCOMPANYBRAND->new();		
	$this->{lines} = $dblist->getADFRAME_USERCOMPANYBRAND($condition,$orderBy,$start,$limit); 
}

sub dbGetCountADFRAME_USERCOMPANYBRAND
{
	my $this			= shift;
	my $condition		= shift;
	require de::adframe::components::company::gen::ListADFRAME_USERCOMPANYBRAND;
	my $dblist = ListADFRAME_USERCOMPANYBRAND->new();	
	$this->{count} = $dblist->getCountADFRAME_USERCOMPANYBRAND($condition);
}
sub readFromFile
{
	my $this	= shift;
...
sub addMessage
{
	my $this	= shift;
	my $mess	= shift;
	$this->{message}.='ClassADFRAME_USERCOMPANYBRAND: '.$mess."\n";   
}
1;