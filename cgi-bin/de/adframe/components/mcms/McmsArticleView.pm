package McmsArticleView;
use de::adframe::components::mcms::gen::CacheADFRAME_MCMS;
...
sub display
{
	...
	if ($mcms->initById($mcmsId))
	{
		...
		$dataHash{CR_DATE_MINUTE}	= $dateT->getMinute(); 
		...
		$this->{adframe}->setSwitch('SWITCH_TEMPLATE_MCMS_IMG_2_1','off');
		$this->{adframe}->setSwitch('SWITCH_TEMPLATE_MCMS_IMG_2_2','off');
		
		if (($dataHash{IMG_1_1} ne '') && ($dataHash{IMG_1_1} !~ /DEFIMG/))
		{
			$this->{adframe}->setSwitch('SWITCH_TEMPLATE_MCMS_IMG_1_1','on');
			$this->{adframe}->setSwitch('SWITCH_TEMPLATE_MCMS_IMG_1_2','off');
		}
		elsif (($dataHash{IMG_1_2} ne '') && ($dataHash{IMG_1_2} !~ /DEFIMG/))
		{
			$this->{adframe}->setSwitch('SWITCH_TEMPLATE_MCMS_IMG_1_1','off');
			$this->{adframe}->setSwitch('SWITCH_TEMPLATE_MCMS_IMG_1_2','on');
		}

		if (($dataHash{IMG_2_1} ne '') && ($dataHash{IMG_2_1} !~ /DEFIMG/))
		{
			$this->{adframe}->setSwitch('SWITCH_TEMPLATE_MCMS_IMG_2_1','on');
			$this->{adframe}->setSwitch('SWITCH_TEMPLATE_MCMS_IMG_2_2','off');
		}
		elsif (($dataHash{IMG_2_2} ne '') && ($dataHash{IMG_2_2} !~ /DEFIMG/))
		{
			$this->{adframe}->setSwitch('SWITCH_TEMPLATE_MCMS_IMG_2_1','off');
			$this->{adframe}->setSwitch('SWITCH_TEMPLATE_MCMS_IMG_2_2','on');
		}
