package CacheListADFRAME_MCMS_RATING;
...
sub getADFRAME_MCMS_RATING
{
	...
	{
		$this->dbGetADFRAME_MCMS_RATING($condition,$orderBy,$start,$limit);
...
sub getCountADFRAME_MCMS_RATING
{
	...
	{
		$this->dbGetCountADFRAME_MCMS_RATING($condition);
...
sub dbGetADFRAME_MCMS_RATING
{
	...
	my $start			= shift;
	my $limit			= shift;
	require C:\www\motox\cgi-bin\de\adframe\components\mcms\gen::ListADFRAME_MCMS_RATING;
	my $dblist = ListADFRAME_MCMS_RATING->new();		
	$this->{lines} = $dblist->getADFRAME_MCMS_RATING($condition,$orderBy,$start,$limit); 
}

sub dbGetCountADFRAME_MCMS_RATING
{
	my $this			= shift;
	my $condition		= shift;
	require C:\www\motox\cgi-bin\de\adframe\components\mcms\gen::ListADFRAME_MCMS_RATING;
	my $dblist = ListADFRAME_MCMS_RATING->new();	
	$this->{count} = $dblist->getCountADFRAME_MCMS_RATING($condition);
}
sub readFromFile
{
	my $this	= shift;
...
sub addMessage
{
	my $this	= shift;
	my $mess	= shift;
	$this->{message}.='ClassADFRAME_MCMS_RATING: '.$mess."\n";   
}
1;