package CacheListADFRAME_MCMS_EXT;
...
sub getADFRAME_MCMS_EXT
{
	...
	{
		$this->dbGetADFRAME_MCMS_EXT($condition,$orderBy,$start,$limit);
...
sub getCountADFRAME_MCMS_EXT
{
	...
	{
		$this->dbGetCountADFRAME_MCMS_EXT($condition);
...
sub dbGetADFRAME_MCMS_EXT
{
	...
	my $dblist = ListADFRAME_MCMS_EXT->new();		
	$this->{lines} = $dblist->getADFRAME_MCMS_EXT($condition,$orderBy,$start,$limit); 
}

sub dbGetCountADFRAME_MCMS_EXT
{
	my $this			= shift;
	my $condition		= shift;
	require de::adframe::components::mcms::gen::ListADFRAME_MCMS_EXT;
	my $dblist = ListADFRAME_MCMS_EXT->new();	
	$this->{count} = $dblist->getCountADFRAME_MCMS_EXT($condition);
}
sub readFromFile
{
	my $this	= shift;
...
sub addMessage
{
	my $this	= shift;
	my $mess	= shift;
	$this->{message}.='ClassADFRAME_MCMS_EXT: '.$mess."\n";   
}
1;