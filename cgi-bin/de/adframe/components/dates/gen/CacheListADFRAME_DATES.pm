package CacheListADFRAME_DATES;
...
sub getADFRAME_DATES
{
	...
	{
		$this->dbGetADFRAME_DATES($condition,$orderBy,$start,$limit);
...
sub getCountADFRAME_DATES
{
	...
	{
		$this->dbGetCountADFRAME_DATES($condition);
...
sub dbGetADFRAME_DATES
{
	...
	my $start			= shift;
	my $limit			= shift;
	require C:\www\motox\cgi-bin\de\adframe\components\dates\gen::ListADFRAME_DATES;
	my $dblist = ListADFRAME_DATES->new();		
	$this->{lines} = $dblist->getADFRAME_DATES($condition,$orderBy,$start,$limit); 
}

sub dbGetCountADFRAME_DATES
{
	my $this			= shift;
	my $condition		= shift;
	require C:\www\motox\cgi-bin\de\adframe\components\dates\gen::ListADFRAME_DATES;
	my $dblist = ListADFRAME_DATES->new();	
	$this->{count} = $dblist->getCountADFRAME_DATES($condition);
}
sub readFromFile
{
	my $this	= shift;
