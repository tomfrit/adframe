package CacheListADFRAME_DATESCATEGORY;
...
sub getADFRAME_DATESCATEGORY
{
	...
	{
		$this->dbGetADFRAME_DATESCATEGORY($condition,$orderBy,$start,$limit);
...
sub getCountADFRAME_DATESCATEGORY
{
	...
	{
		$this->dbGetCountADFRAME_DATESCATEGORY($condition);
...
sub dbGetADFRAME_DATESCATEGORY
{
	my $this			= shift;
	my $condition		= shift;
	my $orderBy			= shift;
	my $start			= shift;
	my $limit			= shift;
	require C:\www\motox\cgi-bin\de\adframe\components\dates\gen::ListADFRAME_DATESCATEGORY;
	my $dblist = ListADFRAME_DATESCATEGORY->new();		
	$this->{lines} = $dblist->getADFRAME_DATESCATEGORY($condition,$orderBy,$start,$limit); 
}

sub dbGetCountADFRAME_DATESCATEGORY
{
	my $this			= shift;
	my $condition		= shift;
	require C:\www\motox\cgi-bin\de\adframe\components\dates\gen::ListADFRAME_DATESCATEGORY;
	my $dblist = ListADFRAME_DATESCATEGORY->new();	
	$this->{count} = $dblist->getCountADFRAME_DATESCATEGORY($condition);
}
sub readFromFile
{
	my $this	= shift;
...
sub addMessage
{
	my $this	= shift;
	my $mess	= shift;
	$this->{message}.='ClassADFRAME_DATESCATEGORY: '.$mess."\n";   
}
1;