package CacheListADFRAME_VOTING_CATEGORY;
$QUAL = '!ADFRAME_VOTING_CATEGORY_ID!|!TITLE!|!TEXT!|!IMG!|!ITEM_0!|!IS_RAFFLE!|!VOTES!|!CR_DATE_YEAR!|!CR_DATE_MONTH!|!CR_DATE_DAY!|!CR_DATE_HOUR!|!CR_DATE_MINUTE!|!END_DATE!|!IS_ACTIVE!';
1;
sub new
{
	my $class			= shift;
...
sub getADFRAME_VOTING_CATEGORY
{
	...
	{
		$this->dbGetADFRAME_VOTING_CATEGORY($condition,$orderBy,$start,$limit);
...
sub getCountADFRAME_VOTING_CATEGORY
{
	...
	{
		$this->dbGetCountADFRAME_VOTING_CATEGORY($condition);
...
sub dbGetADFRAME_VOTING_CATEGORY
{
	my $this			= shift;
	my $condition		= shift;
	my $orderBy			= shift;
	my $start			= shift;
	my $limit			= shift;
	require C:\www\motox\cgi-bin\de\adframe\components\voting\gen::ListADFRAME_VOTING_CATEGORY;
	my $dblist = ListADFRAME_VOTING_CATEGORY->new();		
	$this->{lines} = $dblist->getADFRAME_VOTING_CATEGORY($condition,$orderBy,$start,$limit); 
}

sub dbGetCountADFRAME_VOTING_CATEGORY
{
	my $this			= shift;
	my $condition		= shift;
	require C:\www\motox\cgi-bin\de\adframe\components\voting\gen::ListADFRAME_VOTING_CATEGORY;
	my $dblist = ListADFRAME_VOTING_CATEGORY->new();	
	$this->{count} = $dblist->getCountADFRAME_VOTING_CATEGORY($condition);
}
sub readFromFile
{
	my $this	= shift;
...
sub addMessage
{
	my $this	= shift;
	my $mess	= shift;
	$this->{message}.='ClassADFRAME_VOTING_CATEGORY: '.$mess."\n";   
}
1;