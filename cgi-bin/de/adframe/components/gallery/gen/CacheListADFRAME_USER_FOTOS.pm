package CacheListADFRAME_USER_FOTOS;
...
sub getADFRAME_USER_FOTOS
{
	...
	{
		$this->dbGetADFRAME_USER_FOTOS($condition,$orderBy,$start,$limit);
...
sub getCountADFRAME_USER_FOTOS
{
	...
	{
		$this->dbGetCountADFRAME_USER_FOTOS($condition);
...
sub dbGetADFRAME_USER_FOTOS
{
	...
	my $start			= shift;
	my $limit			= shift;
	require www::V6::cgi-bin::de::fbmx::gen::ListADFRAME_USER_FOTOS;
	my $dblist = ListADFRAME_USER_FOTOS->new();		
	$this->{lines} = $dblist->getADFRAME_USER_FOTOS($condition,$orderBy,$start,$limit); 
}

sub dbGetCountADFRAME_USER_FOTOS
{
	my $this			= shift;
	my $condition		= shift;
	require www::V6::cgi-bin::de::fbmx::gen::ListADFRAME_USER_FOTOS;
	my $dblist = ListADFRAME_USER_FOTOS->new();	
	$this->{count} = $dblist->getCountADFRAME_USER_FOTOS($condition);
}
sub readFromFile
{
	my $this	= shift;
...
sub addMessage
{
	my $this	= shift;
	my $mess	= shift;
	$this->{message}.='ClassADFRAME_USER_FOTOS: '.$mess."\n";   
}
1;