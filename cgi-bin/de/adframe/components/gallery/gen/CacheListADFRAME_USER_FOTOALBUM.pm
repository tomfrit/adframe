package CacheListADFRAME_USER_FOTOALBUM;
...
sub getADFRAME_USER_FOTOALBUM
{
	...
	{
		$this->dbGetADFRAME_USER_FOTOALBUM($condition,$orderBy,$start,$limit);
...
sub getCountADFRAME_USER_FOTOALBUM
{
	...
	{
		$this->dbGetCountADFRAME_USER_FOTOALBUM($condition);
...
sub dbGetADFRAME_USER_FOTOALBUM
{
	my $this			= shift;
	my $condition		= shift;
	my $orderBy			= shift;
	my $start			= shift;
	my $limit			= shift;
	require c:::www::V6::cgi-bin::de::fbmx::gen::ListADFRAME_USER_FOTOALBUM;
	my $dblist = ListADFRAME_USER_FOTOALBUM->new();		
	$this->{lines} = $dblist->getADFRAME_USER_FOTOALBUM($condition,$orderBy,$start,$limit); 
}

sub dbGetCountADFRAME_USER_FOTOALBUM
{
	my $this			= shift;
	my $condition		= shift;
	require c:::www::V6::cgi-bin::de::fbmx::gen::ListADFRAME_USER_FOTOALBUM;
	my $dblist = ListADFRAME_USER_FOTOALBUM->new();	
	$this->{count} = $dblist->getCountADFRAME_USER_FOTOALBUM($condition);
}
sub readFromFile
{
	my $this	= shift;
...
sub addMessage
{
	my $this	= shift;
	my $mess	= shift;
	$this->{message}.='ClassADFRAME_USER_FOTOALBUM: '.$mess."\n";   
}
1;