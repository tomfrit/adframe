package CacheListADFRAME_FOTO_RATING;
...
sub getADFRAME_FOTO_RATING
{
	...
	{
		$this->dbGetADFRAME_FOTO_RATING($condition,$orderBy,$start,$limit);
...
sub getCountADFRAME_FOTO_RATING
{
	...
	{
		$this->dbGetCountADFRAME_FOTO_RATING($condition);
...
sub dbGetADFRAME_FOTO_RATING
{
	...
	my $start			= shift;
	my $limit			= shift;
	require C:\www\V6\cgi-bin\de\adframe\components\gallery\gen::ListADFRAME_FOTO_RATING;
	my $dblist = ListADFRAME_FOTO_RATING->new();		
	$this->{lines} = $dblist->getADFRAME_FOTO_RATING($condition,$orderBy,$start,$limit); 
}

sub dbGetCountADFRAME_FOTO_RATING
{
	my $this			= shift;
	my $condition		= shift;
	require C:\www\V6\cgi-bin\de\adframe\components\gallery\gen::ListADFRAME_FOTO_RATING;
	my $dblist = ListADFRAME_FOTO_RATING->new();	
	$this->{count} = $dblist->getCountADFRAME_FOTO_RATING($condition);
}
sub readFromFile
{
	my $this	= shift;
...
sub addMessage
{
	my $this	= shift;
	my $mess	= shift;
	$this->{message}.='ClassADFRAME_FOTO_RATING: '.$mess."\n";   
}
1;